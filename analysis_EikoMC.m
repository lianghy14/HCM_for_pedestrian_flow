...
temp = zeros(len_y,len_x,MC_max);
for i = 1:MC_max
    temp(:,:,i) = Record_q1{i,2};
end
MC_max_mean = mean(temp,3);
MC_max_std = std(temp,0,3);

MC_seq_mean = zeros(len_y,len_x,length(MC_nseq));
MC_seq_std = zeros(len_y,len_x,length(MC_nseq));
...
for i = 1:length(MC_nseq)
    ...
    temp = zeros(len_y,len_x,MC_nseq(i));
    for j = 1:MC_nseq(i)
        temp(:,:,j) = Record_q1{j,2};
    end
    MC_seq_mean(:,:,i) = mean(temp,3);
    MC_f_mean(i) = mean(MC_fin);
    MC_seq_std(:,:,i) = std(temp,0,3);
...
RRMSE_valid = (sum(sum(bj_ex3ODH>2.5)));

for i = 1:length(MC_nseq)
    RRMSE_mean(i) = sqrt( gfuns.RRMSE( (MC_seq_mean(:,:,i) - MC_max_mean(:,:)),2,RRMSE_valid) )./gfuns.RRMSE(MC_max_mean(:,:),1,RRMSE_valid);
    RRMSE_std(i) = sqrt( gfuns.RRMSE( (MC_seq_std(:,:,i) - MC_max_std(:,:)),2,RRMSE_valid) )./gfuns.RRMSE(MC_max_std(:,:),1,RRMSE_valid);
end
